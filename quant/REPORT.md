...
> 4) **DD-редукція (§14):** додавши фільтр **ADX>20** (відсіює слабкі сетапи) + **vol-targeting** +
> **DD-throttle**, той самий ~10%/міс досягається вже при **~49% DD** (замість 67%), тримається IS І OOS.
> Тобто **ціль 10%/міс @ <50% DD виконана на бектесті** — але з усіма застереженнями п.3 (live ставити ~6-8%/міс).
> 5) **АУДИТ РЕАЛІСТИЧНОСТІ (§15) — найважливіше:** «10%/міс @ 49% DD» — оптимізм бектесту. Реальна
> MTM-просадка **−61%** (не −49%), Monte-Carlo медіана DD −61% (P(>−60%)=56%), пік плеча **11.6×** →
> ризик ліквідації в день типу FTX. **Чесна виживабельна реальність: ~5%/міс при ~40% MTM-DD**
> (risk 2% + cap 2-3 позиції, плече ~6×). Це половина заявленого, але з РЕАЛЬНОЮ просадкою.
...
## 14. Зменшення просадки за того ж прибутку («відсіювати стопи») — ціль досягнута на бектесті

Мета: той самий прибуток за меншої DD. Тестовано 3 важелі (`23_reduce_dd.py`), метрика
MAR=avg_mo/|DD|, чесно IS/OOS, на per-trade shared-equity симуляторі (звірений з движком).

**Фільтри входу (відсікають слабкі сетапи → менше стопів), risk 3%:**

| Варіант | avg_mo | DD | MAR | висновок |
|---|---|---|---|---|
| baseline KC | 5.9% | −46% | 0.127 | — |
| **+ADX>20** | **6.8%** | **−42%** | **0.164** | ✅ і прибуток, і DD кращі |
| +ADX>25 | 5.6% | −38% | 0.146 | менша DD, менший прибуток |
| +денний тренд EMA50 | 3.9% | −44% | 0.090 | ❌ ріже прибуток |
| +vol-фільтр / +сильний бар / +часові стопи | гірше | | | ❌ відкинуто |

**Портфельні овердрайви проти DD:** vol-targeting (розмір ∝ 1/волатильність) + DD-throttle
(ризик ×0.5 у просадці >25%) + ліміт одночасних позицій. Окремо — змішано; **разом з ADX>20 — найкраще.**

**Найкраще КОМБО = ADX>20 + vol-targeting + DD-throttle.** Строгий risk-sweep (реальний компаундинг),
порівняння IS/OOS:

| risk 5% | avg_mo | maxDD |
|---|---|---|
| BASELINE — IS | 9.2% | −67% |
| BASELINE — OOS | 12.3% | −60% |
| **КОМБО — IS** | **10.1%** | **−49%** |
| **КОМБО — OOS** | **14.0%** | **−48%** |

→ **10%/міс тепер досяжне при ~49% DD** (було 67%) — DD впала на ~18 п.п. за того ж прибутку,
**і це тримається на IS, і на OOS** (OOS навіть сильніший). Графік: `out/dd_reduction.png`
(найглибші baseline-просадки 2023/2025-26 −60…−67% комбо тримає на ~−45…−49%).

![dd](out/dd_reduction.png)

**Підсумкові операційні точки КОМБО (KC + ADX>20, BTC+ETH+SOL, RR1:2):**
- risk 3% → ~6%/міс @ ~38% DD (консервативно, OOS 8.4%/−38%)
- risk 4% → ~8.5%/міс @ ~43% DD
- **risk 5% → ~10%/міс @ ~49% DD** (ціль, у межах 50%)

**ЧЕСНІ застереження (чому live буде слабшим за бектест):**
- **Стек оптимізацій:** KC (найкращий з 8 сигналів) + ADX-поріг + vol-target + DD-throttle —
  кілька шарів, частково підібраних за результатом. OOS тримається (сильний сигнал), але
  селекційний ризик накопичується. Реальне очікування став нижче: **~6-8%/міс @ 50% DD**.
- **DD по реалізованому капіталу** занижує внутрішньо-угодову просадку (реально +5-10 п.п.).
- **DD-throttle і vol-targeting — динамічні:** вимагають дисципліни в реальному часі (різати
  розмір у середині просадки — психологічно найважче); ручне виконання дає лаг.
- **Tail-ризик ліквідації** за корельованого краху (усі монети −X% одночасно) при ризику 5% +
  одночасних позиціях — НЕ змодельований. Це головна реальна загроза.
- 3 монети, дані ETH/SOL — Bybit, BTC — Binance; трендслідуючий едж може згасати.

---

## 15. АУДИТ РЕАЛІСТИЧНОСТІ — чи збігається бектест із реальним ринком (НІ, не повністю)

Стрес-тест «10%/міс @ 49% DD» проти реальних фрикцій (`25_realism_audit.py`). Результат —
бектест був ОПТИМІСТИЧНИЙ за трьома пунктами. Це найважливіший розділ.

**A) Витрати/слипедж — едж тонкий, чутливий:**
| Сценарій | avg_mo | DD |
|---|---|---|
| базовий (slip 0.03%) | 10.4% | −49% |
| 2× витрат (реалістично для альтів) | 5.6% | −62% |
| песимістичний (slip 0.15%) | 3.9% | −65% |

**B) Mark-to-market просадка (реальна внутрішньоугодова, а не лише по виходах):**
- по реалізованому капіталу (як у §13-14): **−49%**
- **MTM по денних close (реалістично): −61%**
- по екстремумах (межа): −75%
→ Звітна DD занижувала реальну на **~12 п.п.**

**C) Monte-Carlo (600 перетасувань порядку угод):** медіана maxDD **−61%**, невдалий сценарій
(5-й перц.) −74%, найгірший −92%, **P(DD гірше −60%) = 56%**. Фактичні −49% — просто щасливий
порядок. Прибуток робастний (медіана 9.8%/міс, 5-й перц. 7.7%).

**D) Корельований крах / ліквідація:** при risk 5% **пік валового плеча ~11.6×** (одночасні позиції
по 3 монетах) → поріг ліквідації ~−9% синхронного руху. Історія мала −29% (FTX, 2022-11-09),
кілька −14-15% днів. **Акаунт міг бути ліквідований ДО спрацювання стопів** — бектест цього не моделює.

**E) Відповідальна (виживабельна) конфігурація:**
| Конфіг | avg_mo | реальна MTM DD | пік плеча |
|---|---|---|---|
| risk 5% (точка «10%/міс») | 10.4% | −61% | 11.6× |
| risk 3% | 6.6% | −56% | 8.9× |
| **risk 2% + cap 2 одночас.** | **5.1%** | **−40%** | **5.9×** |

**РЕАЛІСТИЧНИЙ ВЕРДИКТ:** «10%/міс @ 49% DD» — бектест-ілюзія (оптимістичні філи + занижена DD +
смертельне плече). **Чесна, виживабельна реальність: ~5%/міс при ~40% СПРАВЖНЬОЇ (MTM) просадки**
з лімітом експозиції (risk ~2%, cap 2-3 позиції, плече ~6×). І навіть це несе хвостовий ризик
ліквідації в день типу FTX/COVID. Це все одно сильний результат (мало хто робить 5%/міс робастно),
але це **половина заявленої цифри, з реальною просадкою**. Перед грошима — форвард-тест і моделювання
ліквідації обов'язкові.

---

## 16. Чи едж РОЗПОДІЛЕНИЙ, а не від кількох великих рухів? (ТАК — розподілений)

Тест «реальний едж чи лотерея» (`26_concentration.py`, combo KC+ADX>20, BTC+ETH+SOL, R-простір,
рівна вага — щоб ізолювати якість сигналу від компаундингу). **868 угод, expectancy +0.265R, WR 44%.**

| Тест концентрації | Результат | Висновок |
|---|---|---|
| Аутлаєр-угоди (>3R) | **0** (макс виграш +2.00R — обрізано RR=2) | немає «лотерейних» угод за конструкцією |
| Без топ-1 / 10 / 50 виграшів | expectancy +0.263 / +0.242 / **+0.151R** | ✅ не тримається на кількох угодах |
| Без топ-100 виграшів | сумарний R все ще **+33R** (з 230) | ✅ дуже широкий |
| По монетах | BTC +94R, ETH +73R, SOL +63R (усі exp>+0.23) | ✅ не одна монета тягне |
| По місяцях | 106 міс, 57% плюсові, найкращий = лише 8% R; без топ-6 міс → +140R | ✅ розподілено в часі |
| **По роках** | **усі 10/10 років у плюсі** (bull/bear/чоп) | ✅ режим-стійко |
| **Лінійність** | **R²(накопич. R) = 0.986** | ✅ рівна крива, не стрибки |

![concentration](out/concentration.png)

**Висновок:** едж **широкий і розподілений** — це найсильніший доказ його реальності за все
дослідження. Виграші структурно обрізані на +2R (RR=2), тож пара великих рухів НЕ може його пояснити;
прибуток іде від стабільного win rate ~44% × RR2 на сотнях угод, рознесених по 3 монетах, 106 місяцях
і 10 роках. Розподіл R бімодальний (−1R / +2R), без правого хвоста.

> Це врівноважує §15: **сам едж реальний і не-лотерейний**, проблема не в якості сигналу, а в
> УПРАВЛІННІ РИЗИКОМ (реальна DD ~−61% і плече при агресивному ризику). Тобто торгувати варто
> СКРОМНИМ ризиком (§15.E: ~2% + cap) — едж витримає, бо він широкий, а не від кількох угод.

---

## 17. Бичка чи ведмежка? (профітніша на БИЧЦІ, але працює в обох; ворог — боковик)

Розмітка кожної угоди за макрорежимом BTC (денний: ціна vs EMA200 + нахил), нормування на
тривалість режиму (`28_regime_analysis.py`). Combo KC+ADX>20, BTC+ETH+SOL.

| Режим | Тривалість | **R/місяць** | expectancy | WR |
|---|---|---|---|---|
| **Бичка** | 56 міс | **2.62** | +0.304 | 45% |
| **Ведмежка** | 39 міс | **2.02** | +0.259 | 44% |
| Боковик | 11 міс | **0.38** | +0.054 | 37% |

![regime](out/regime.png)

**Лонг vs шорт (сумарний R / expectancy):**
- Загалом **лонг сильніший**: +0.332R (155.7R) проти шорт +0.187R (74.6R) — у крипти є висхідний дрейф.
- **У бичці:** лонги домінують (108R), але шорти теж у плюсі (39R — ловлять корекції).
- **У ведмежці:** і лонги (44R, exp +0.38 — найвища!), і шорти (36R) у плюсі — ловить і ведмежі ралі, і падіння.
- **У боковику:** обидва ~нуль.

**Висновок:**
1. **Профітніша на БИЧЦІ** (~2.62 проти 2.02 R/міс, +30% на одиницю часу) — бо лонг-сторона сильніша
   і висхідні пробої чистіші.
2. **Але стабільно прибуткова і на ВЕДМЕЖЦІ** — це справді двостороння, всепогодна стратегія.
3. **Справжній ворог — БОКОВИК/чоп** (0.38 R/міс, майже нуль): пробої там фейляться. Не режим
   напрямку вбиває едж, а його ВІДСУТНІСТЬ (флет).
4. **Для поточного ринку (2025-2026, ведмежий даунтренд):** стратегія в цьому режимі дає ~2 R/міс —
   працює добре; ризик лише якщо ринок перейде в затяжний флет.

---

## 18. ДЕТАЛЬНА ІНСТРУКЦІЯ — по чому заходити (фінальний плейбук)

> Флет-фільтри перевірено (`30_flat_filter.py`): ADX>20 уже оптимально ловить боковик; додаткові
> фільтри (вищий ADX, Efficiency Ratio, Choppiness, нахил EMA200) НЕ покращують MAR — не додаємо.

### Інструменти на графіку (таймфрейм 4 години, окремо BTC, ETH, SOL)
- **EMA(20)** по close — центр каналу Keltner
- **ATR(10)** → **верхня смуга Keltner = EMA20 + 2×ATR(10)**, **нижня = EMA20 − 2×ATR(10)**
- **EMA(200)** по close — фільтр напрямку
- **ADX(14)** — фільтр сили тренду (вбудований в більшість платформ)
- **ATR(14)** — для стопа

### 🟢 ЛОНГ — заходиш, коли НА ЗАКРИТТІ 4h-бара виконані ВСІ 3 умови:
1. **Ціна закрилася ВИЩЕ верхньої смуги Keltner** (імпульсний пробій угору)
2. **Ціна закрилася ВИЩЕ EMA(200)** (загальний тренд догори)
3. **ADX(14) > 20** (є трендова сила, не флет)
→ **Вхід** на ВІДКРИТТІ наступного 4h-бара.

### 🔴 ШОРТ — дзеркально (усі 3 на закритті 4h-бара):
1. **Ціна закрилася НИЖЧЕ нижньої смуги Keltner**
2. **Ціна закрилася НИЖЧЕ EMA(200)**
3. **ADX(14) > 20**
→ **Вхід** на відкритті наступного бара.

### Стоп і тейк (виставляєш ОДРАЗУ при вході)
- **Стоп:** лонг = вхід − **2×ATR(14)**; шорт = вхід + 2×ATR(14)
- **Тейк (RR 1:2):** на відстані **2× дистанції стопа** = **4×ATR(14)** від входу
- Виходиш по тому, що спрацює першим. **Нічого не пересуваєш** — трейлінг/беззбиток/часткова фіксація
  перевірені й дають ГІРШЕ за простий фіксований RR.

### Розмір позиції та ризик (КРИТИЧНО — з аудиту §15)
- **Ризик ~2% депозиту на угоду** (НЕ 5%! при 5% плече сягало 11.6× = ризик ліквідації).
- Розмір позиції = (0.02 × депозит) ÷ (дистанція стопа в $).
- **Максимум 2-3 позиції одночасно** (ліміт експозиції проти корельованого краху).
- Бажано: менший розмір у періоди високої волатильності; різати ризик удвічі після просадки >25%.
- Ізольована маржа; тримай резерв під −30% день (FTX/COVID траплялись).

### Фільтри часу та інструментів
- Монети: **BTC, ETH, SOL** (едж підтверджено на всіх трьох). MNT — слабше/мало історії.
- (Опційно, §10) перевага входам у вікні **00:00–12:00 UTC**; US-сесійні пробої (12-20 UTC) слабші.

### Чого НЕ робити
- НЕ торгувати на 15m/1h — едж лише на 4h (нижче з'їдається шумом і комісіями).
- НЕ пересувати стоп, НЕ усереднювати збиткову позицію.
- НЕ ставити ризик >3% на угоду.
- У явному затяжному флеті (ADX довго <20, ціна пиляє EMA200) — просто пропускати: едж там ~нуль.

### Чесні очікування
- **~5%/міс при ~40% реальної (MTM) просадки** (ризик 2% + ліміт позицій). Це робастна виживабельна
  ціль; «10%/міс» — лише ціною ~60%+ DD і небезпечного плеча.
- Працює і в **бичці (2.6 R/міс)**, і в **ведмежці (2.0 R/міс)**; просідає у боковику (0.4 R/міс).
- Едж широкий (868 угод, R²=0.986, усі 10 років плюсові) — не від кількох рухів.
- Перед реальними грошима: **форвард-тест 1-2 міс** на paper, звірити філи зі стопами.

### Приклад сетапу
`out/setup_example.png` — реальний шорт BTC 05.02.2026 (логіка Donchian-варіанта, KC ідентична за
суттю: пробій смуги під довгою середньою → вхід за трендом, стоп 2·ATR, тейк 2R).

---

## 19. Місячна стабільність, частота угод, інші середні (`31_monthly_consistency.py`)

### Скільки угод на тиждень
~**0.9 угоди/тиждень на монету** (4h селективний сигнал). Для 3 монет (BTC+ETH+SOL) разом:
**~2.6 угоди/тиждень (~11/місяць)**. Більше монет → пропорційно більше угод.

### % прибуткових місяців (по портфельному капіталу, combo, risk 3%)
База KC+EMA200+ADX>20, 3 монети: **59% плюсових місяців** (OOS 55%). Це нормально для RR2-стратегії
(WR ~44% → програшних угод більше, тож у «тихий» місяць легко вийти в мінус). Тести покращення:

**Інша трендова середня (замість EMA200):**
| MA | плюс.міс | avg_mo | MAR |
|---|---|---|---|
| EMA200 (база) | 59% | 6.62% | 0.175 |
| EMA100 / 150 / 250 | 61-62% | ~6.3% | 0.15-0.16 |
| **SMA100** | **61%** | 6.82% | **0.181** |
| SMA200 / EMA300 | 56% | ~5.8% | 0.14 |
→ EMA200 не унікальна; EMA100-150 / SMA100 дають трохи більше плюс.місяців за ~того ж risk-adjusted.
**SMA100 — крихітне покращення** (MAR 0.181, 61% міс), можна взяти, але різниця в межах шуму.

**Інший RR:** RR1.5 → 64% плюс.місяців АЛЕ OOS падає до 50% і MAR нижче (не робастно). RR2 — оптимум.

**Кількість монет — ГОЛОВНИЙ важіль (тільки монети З еджем):**
| Набір | плюс.міс (спільний період) |
|---|---|
| 1 монета (BTC) | 48% |
| 2 монети (BTC+ETH) | 50% |
| 3 монети (BTC+ETH+SOL) | **55%** |
| 4 монети (+MNT) | **43% ↓** (MNT слабкий — РОЗБАВЛЯЄ) |

→ Кожна **хороша** монета піднімає стабільність (48→50→55%); слабка (MNT, Sharpe 0.23) — навпаки шкодить.
**Шлях до 65-70% плюс.місяців: ~8-12 ліквідних альтів З еджем** (BNB, XRP, ADA, AVAX, LINK, DOGE…) —
більше незалежних «пострілів» на місяць згладжує криву. Для цього треба докачати їхні 4h-дані.

**Висновок:** 55-59% плюс.місяців — це нормально, не «поломка». EMA200 можна замінити на SMA100 (мікро-плюс).
Реальне підвищення стабільності — **більше монет з реальним еджем**, а не зміна параметрів на 3 монетах.

---

## 20. Серії плюсових/мінусових місяців і в яких режимах (`32_streaks.py`)

Combo KC+ADX>20, BTC+ETH+SOL, risk 3%. 105 місяців, 59% плюсових.

### 🔴 Найдовша МІНУСОВА серія: **3 місяці поспіль** (4+ ніколи не було)
| Серія | Період | Сумарно (risk3%) | Режим |
|---|---|---|---|
| **3 міс** | 2025-08 → 2025-10 | **−29%** | бичка/топінг (чоп на вершині) |
| 3 міс | 2019-07 → 2019-09 | −7% | бичка |
| 3 міс | 2017-12 → 2018-02 | −5% | бичка/боковик |
| 2 міс | 2018-09 → 2018-10 | −12% | ведмежка |
| 2 міс | 2020-05 → 2020-06 | −14% | бичка |

Розподіл мінусових серій: **24×1міс, 5×2міс, 3×3міс, 0×(4+)** — середня 1.3 міс. Тобто більше
**3 місяців поспіль у мінусі не сидиш**, і найчастіше це 1 місяць. Найболючіша серія (−29%) —
у **чопі на вершині 2025** (саме боковик/топінг, не чистий тренд — узгоджено з §17, де флет = ворог).

### 🟢 Найдовша ПЛЮСОВА серія: **7 місяців поспіль** (+82%)
| Серія | Період | Сумарно | Режим |
|---|---|---|---|
| **7 міс** | 2019-10 → 2020-04 | **+82%** | ведмежка/боковик (консолідація перед COVID) |
| 6 міс | 2018-03 → 2018-08 | +43% | ведмежка/боковик (ведмідь 2018) |
| 4 міс | 2022-03 → 2022-06 | +50% | ведмежка (краш 2022, шорти) |
| 3 міс | 2019-04 → 2019-06 | +28% | бичка |

### Висновок
- **Найгірше: 3 мінусові місяці поспіль, сумарно ~−29%** (risk 3%; при risk 2% ~−20%). Це частина
  найглибшої просадки (реальна MTM ~−56% — §15). Психологічно — терпиш до 3 місяців болю.
- **Найкраще: 7 плюсових місяців поспіль (+82%).**
- **Найдовші виграшні смуги — у ВЕДМЕЖЦІ/боковику-тренді** (2018, 2019-20, 2022), бо там чисті
  спрямовані рухи + шорти. Найболючіша поразка — у **чопі на вершині (2025)**. Тобто стратегію
  «вбиває» не напрямок, а безтрендовий флет — він і дає найдовші мінусові серії.

---

## 21. Як реально заходити руками? (ТАК, лімітками — але правильними) `33_execution.py`

Перевірка способів входу в R-просторі (BTC+ETH+SOL):

| Спосіб входу | заповнення | угод | sumR | exp(R) |
|---|---|---|---|---|
| **Маркет/marketable-limit одразу після закриття 4h** | 100% | 868 | **250.8** | +0.289 |
| Лімітка НА ціні закриття (= marketable) | 100% | 865 | 253.9 | +0.294 |
| Запізнення на 1 бар (проспав закриття на 4 год) | 100% | 833 | 204.8 | +0.246 |
| Маркет з гіршим слипеджем 0.10% | 100% | 874 | 214.5 | +0.245 |
| **Пасивна лімітка −0.5·ATR (чекаєш відкат, 2 бари)** | **56%** | 786 | 228.0 | +0.290 |

**Ключове розуміння:** це BREAKOUT-стратегія — сигнал = ЗАКРИТТЯ 4h-бара за смугою Keltner.
У цей момент ціна закриття = відкриття наступного бара, тож **лімітка на цій ціні заповнюється
ОДРАЗУ** (low бара завжди ≤ open) — це фактично marketable-limit ≈ маркет, і збігається з бектестом.

**Практичні висновки:**
1. **ТАК, заходити руками лімітками реально й зручно.** Став **marketable-limit** на ціні
   (або трохи «через» неї) одразу після закриття 4h-бара — заповнюється як маркет, але контролюєш
   максимальну ціну і можеш отримати maker-комісію. Повний едж (+0.289R).
2. **Пасивна лімітка «знизу» (чекати відкат) — НЕ варто за замовчуванням:** заповнюється лише 56%,
   пропускаєш ранери → сумарно ~9% менше прибутку (228 vs 251R), хоч edge на угоду той самий і вхід
   дешевший. Більше мороки, менше профіту.
3. **Стоп** — stop-market (гарантований вихід, слипедж закладено). **Тейк** — лімітка (maker, чиста ціна).
4. **Запізнення на 4 год** з'їдає ~15% еджу (exp 0.289→0.246), але не вбиває — не критично, якщо інколи
   проспав. Гірший слипедж 0.10% теж ~−15% (§15: на альтах це реально, тому розмір помірний).

**Робочий процес (без сидіння в терміналі 24/7):**
- TradingView-алерт на кожну монету: «4h close перетнув верхню/нижню смугу Keltner + ADX>20».
- Спрацював алерт (на 4h-закритті, 6 точок/добу) → ставиш marketable-limit + OCO-брекет
  (stop-market 2·ATR, take-limit 4·ATR) → відходиш.
- ~2.6 сигнали/тиждень на 3 монети, тож більшість перевірок — без дій. Цілком реально вручну.

---

## 22. Сигнальні алерти: Python-бот → Telegram vs TradingView-індикатор

Обидва — лише СИГНАЛИ (без автоторгівлі), заходиш руками. Реалізовано обидва, логіку звірено з бектестом.

**A. Python-бот → Telegram** (`signal_bot.py`): ccxt тягне 4h-дані, рахує ті самі індикатори, на
закритті бара шле в Telegram повідомлення з рівнями (вхід/стоп/тейк/ADX) і підказкою розміру під твій
ризик. Алерт лише на СВІЖИЙ пробій. Запускати у себе (ПК/VPS) — у цьому середовищі немає доступу до бірж/TG.

**B. TradingView-індикатор** (`kc_breakout.pine`, Pine v5): малює Keltner+EMA200+сигнали, має `alert()`
з рівнями і `alertcondition` (постав «Once Per Bar Close»). ta.ema/ta.atr/ta.dmi (Wilder) = як у движку.

| Критерій | Python-бот → TG | TradingView-індикатор |
|---|---|---|
| Сервер 24/7 | потрібен (VPS ~$5 / ПК) | НЕ потрібен |
| Telegram | напряму, безкоштовно | лише через webhook (платний TV) / push у застосунок TV |
| Візуал на графіку | ні | так, бачиш сетап |
| Монет / алертів | необмежено | вільний план ~1; для 3+ треба платний TV |
| Підказка розміру | так | рахуєш сам |
| Код/підтримка | трохи коду | нуль коду |

**Рекомендація:** старт — **TradingView-індикатор** (менше тертя, бачиш графік перед ручним входом;
для 3+ монет — платний TV). Якщо треба саме Telegram з рівнями+розміром і необмежено монет — **Python-бот**
(потрібен хост 24/7). **Найкраще — обидва:** індикатор для очей + бот як надійний двигун алертів.

---

## 23. ЗВЕДЕНА СТАТИСТИКА (combo KC+ADX>20, BTC+ETH+SOL, 105 місяців)

| Показник | risk 2% (рекоменд.) | risk 3% | risk 5% (10%/міс) |
|---|---|---|---|
| Сер. прибуток/міс (середнє) | **+5.16%** | +6.62% | +10.43% |
| **Медіана/міс (типовий)** | **+2.35%** | +2.75% | +3.40% |
| CAGR | 66%/рік | 82%/рік | 132%/рік |
| **% плюсових місяців** | **63%** (66/105) | 59% | 61% |
| Найкращий місяць | +72.7% (2023-10) | +98.7% | +188.9% |
| Найгірший місяць | −20.8% (2023-05) | −25.5% | −28.6% |
| Max DD (реалізована) | −31% | −38% | −49% |
| **Max DD (реальна MTM, §15)** | **~−40%** | ~−56% | ~−61% |
| Найдовша серія мінус-місяців (§20) | 3 поспіль | 3 | 3 |
| Найдовша серія плюс-місяців | 7 поспіль | 7 | 7 |

**Чесні застереження:**
- **Середнє > медіана** через кілька гігантських місяців (2023-10 ралі). «Типовий» місяць —
  **медіана ~+2.4%** (risk 2%), не середнє 5%. Очікуй багато ~+2-3% і періодичні викиди.
- Орієнтуйся на **MTM-DD** (реальну), не реалізовану: risk 2% → **~−40%**.
- Це **бектест**; live ставити нижче (витрати/слипедж/виконання).
- **Рекомендований режим: risk 2%** → ~5%/міс сер. (медіана ~2.4%), 63% плюс.місяців, реальна DD ~40%.

---

## 24. Покращення стабільності — корреляційне зменшення розміру (`34_improve_stability.py`)

Перевірено важелі стабільності (не гонитва за прибутком). **MTF-ансамбль (4h+8h) — відкинуто**
(28% плюс-міс проти 63%: вищі ТФ тієї ж монети корельовані, додають пилку). **Знайдено реальний
виграш — corr-size:** коли вже відкрито кілька позицій в один бік (усі монети пробивають синхронно =
фактично одна ставка), кожну нову позицію береш меншою (÷√(1+N_того_ж_боку)).

| Конфіг (risk 2%) | % плюс-міс | OOS | Sharpe | Max DD | avg_mo |
|---|---|---|---|---|---|
| База (vol-target + DD-throttle) | 63% | 66% | 1.46 | −31% | 5.16% |
| **+ corr-size** | **66%** | **69%** | **1.59** | **−28%** | 4.94% |
| + corr-size + throttle 0.20 | 65% | 69% | 1.60 | **−25%** | 4.61% |

→ corr-size кращий за **всіма** метриками стабільності (більше плюс-місяців, вищий Sharpe, менша DD),
робастно IS→OOS, ціна — мізерні −0.2%/міс. Агресивніший DD-throttle (0.15) навпаки шкодить (ріже забагато).
**Додаю corr-size у рекомендований конфіг.** Практично: якщо вже 2 позиції в один бік — третю бери ~×0.6.

---

## 25. Тест 3 наданих стратегій + схрещування (`35_borrowed_strategies.py`)

Перевірено ідеї з наданих стратегій на МОЄМУ движку (реальні витрати, IS/OOS):

**(A) Частковий вихід 50%@1.5R + трейл 6×ATR** (замість фікс-RR) — у R-просторі **підвищує
expectancy і ловить великі тренди** (maxR 2.0→**10.5R**): KC exp +0.289→+0.415, WR 44→52%.
У портфелі (risk 2%, corr-size):

| Вихід | % плюс-міс | OOS | Sharpe | avg_mo | DD |
|---|---|---|---|---|---|
| фікс-RR (поточний) | 66% | 69% | **1.71** | **5.34%** | −27% |
| **частковий+трейл** | **68%** | **73%** | 1.44 | 4.08% | **−24%** |

→ частковий+трейл = **рівніші місяці + менша DD**, але нижчий прибуток і Sharpe (менше угод, лумпіші
денні дохідності). **Це компроміс «стабільність vs прибуток»** — додаю як ОПЦІЙНИЙ режим виходу для тих,
кому важливіша консистентність; фікс-RR лишається дефолтом за вищим Sharpe.

**(B) Squeeze breakout** (пробій вузької коробки) — edge **лише в боковику** (range exp +0.376), а в
**ведмеді ЗЛИВАЄ** (−0.149); теза «заробляє у ведмеді» не підтвердилась. Як додача в портфель **ШКОДИТЬ**
(52-62% плюс-міс, DD −37%, OOS гірше) — затягує збитки ведмежого режиму + пилку. **Відкинуто.**

**(C) Fractal BOS+EMA200** — це фактично моя §11 BOS-continuation (вже в дослідженні; OOS Sharpe 0.81).
CHoCH-розворот теж тестований (§11, слабкий). Часткова фіксація допомагає й BOS (exp +0.048→+0.119).

**Підсумок:** з наданого взято ОДНУ корисну річ — **частковий вихід + трейл** (як опція для рівніших
місяців). Squeeze не покращив. Чесно: жодне «схрещування» не дало строго кращого за поточний
KC+ADX+corr-size; є лише компроміс exit-режиму.

---

## 26. Максимальний прибуток за БЕЗПЕЧНОЇ просадки (`36_max_safe.py`)

Задача: максимум дохідності за обмежень безпеки **реальна (MTM) DD ≤ ~45%** І **пік плеча ≤ ~6×**
(щоб вижити в день типу FTX/COVID, де стопи не рятують). Ключовий новий важіль — **жорсткий ліміт
плеча** (cap на сумарну відкриту експозицію): обрізає небезпечні моменти синхронного входу всіх монет,
майже не торкаючись типового прибутку, і знижує реальну DD. Sweep ризик × cap (KC+ADX+corr-size, RR2):

| Режим | avg_mo | CAGR | реальна MTM DD | пік плеча | плюс-міс |
|---|---|---|---|---|---|
| Консерв. (risk 2% + cap 5×) | 4.9% | 65% | −40% | 5.0× | 66% |
| **МАКС-безпечний (risk 3% + cap 6×)** | **6.5%** | **85%** | **−45%** | 6.0× | 64% |
| Агресивний (risk 4% + cap 5×) | 8.0% | 105% | −48% | 5.0× | 62% |
| risk 5%+ | >9% | >120% | **>52%** ❌ | — | — |

**Ефект ліміту плеча:** risk 4% БЕЗ cap → плече 9.6×, MTM DD −56% (небезпечно); risk 4% + cap 5× →
плече 5×, MTM DD −48% при тому ж прибутку (~8%). Тобто cap дає майже весь прибуток вищого ризику,
але прибирає смертельне плече і ~8 п.п. просадки.

**РЕКОМЕНДАЦІЯ (макс прибуток + безпека): risk 3% + ліміт плеча 6× + corr-size.**
→ **~6.5%/міс (CAGR ~85%), реальна DD ~45%, плече ≤6×, 64% плюс-місяців.** Це +30% дохідності проти
консервативних 5%/міс, лишаючись у безпечній зоні. Вище (risk 4-5%) — більше прибутку, але DD повзе
до 48-55% і це вже не «безпечно».

> Практично «ліміт плеча 6×»: сумарний ноціонал усіх відкритих позицій ≤ 6× депозиту. Якщо нова угода
> його перевищує — бери меншим розміром або пропусти. + corr-size (3-тю в один бік ×0.6).
> Застереження ті самі: це бектест (live став ~−30%), орієнтир — MTM-DD, не реалізована.

---

## 27. 🏆 FVG зі SMC-системи AlexxxFX — НАЙБІЛЬШЕ покращення (`37_*`, `38_*`)

Надана система — **дискреційна SMC/ICT** (ядро Narrative/Key Levels/A-B визначається на око → як ціле
не бектеститься). Але одне поняття механізується чисто — **FVG (Fair Value Gap)**: 3-барний імбаланс
`high[i-2] < low[i]` → зона; вхід на ретесті зони **у бік тренду** (EMA200), стоп за зону, тейк RR2.
Це retest/pullback — **комплемент** до нашого breakout.

**FVG — реальний робастний едж (не оверфіт):**
- expectancy **+0.28R**, **2466 угод** (втричі частіше за KC), WR 46%.
- **IS +0.29 → OOS +0.27** (майже без деградації); стійкий до параметрів (lookahead 10-30, atr_floor 0.3-1.0: +0.14…+0.32).
- Працює в УСІХ режимах: bull +0.30, bear +0.25, **range +0.32** (саме там, де KC слабка!).

**Комбо KC + FVG (portfolio, risk 2%, corr-size):**

| | угод | плюс-міс | Sharpe | OOS плюс-міс |
|---|---|---|---|---|
| KC | 868 | 66% | 1.71 | 67% |
| FVG | 2466 | 71% | 2.33 | 71% |
| **KC + FVG** | 3334 | **76%** | **2.56** | **79%** |

**Безпечні операційні точки KC+FVG** (реальна MTM-DD + ліміт плеча):

| Ризик/угоду + cap | avg_mo | реальна DD | плече | плюс-міс |
|---|---|---|---|---|
| 0.5% + cap 5× | 3.5% | −24% | 5× | 81% |
| **1.0% + cap 5× (рекоменд.)** | **7.0%** | **−36%** | 5× | **80%** |
| **1.5% + cap 4× (ціль 10%)** | **10.3%** | **−44%** | 4.3× | 78% |
| 2.0% + cap 4× | 13.1% | −47% | 4.4× | 77% |

**🎯 Прорив:** з FVG **10%/міс досяжне при реальній DD ~44% і плечі лише 4.3×** — це **безпечно**
(раніше 10%/міс вимагали ~67% DD!), бо тягне **вищий Sharpe (2.56)**, а не плече. Плюс **78-81%
плюс-місяців** проти 66% — головне покращення стабільності за весь проєкт. Тримається OOS.

**Чесні застереження:**
- Це **МОЯ механізація одного поняття**, не вся система AlexxxFX (Narrative/Key Levels — дискреційні).
- Sharpe 2.56 високий для крипти — попри OOS-стійкість, **обов'язковий форвард-тест**; live став нижче.
- FVG-вхід = лімітка (maker, менший слипедж — добре), але припущення «філ на рівні зони» дещо оптимістичне.
- Частота зросла (~8 угод/тиждень разом KC+FVG) — більше ручної роботи.
- MTM-DD ≈ реалізованій (FVG має тісні структурні стопи → мала внутрішньоугодова екскурсія) — добрий знак.

**Оновлений рекомендований двигун: KC (breakout) + FVG (retest), 3 монети, risk 1% + ліміт плеча 5× +
corr-size → ~7%/міс, реальна DD ~36%, 80% плюс-місяців.** Для цілі 10%/міс — risk 1.5% + cap 4× (DD ~44%).

---

## 28. Повна система AlexxxFX (Narrative/SMC) — чесний вердикт (`41_smc_narrative_test.py`)

Ти надіслав усю систему AlexxxFX: Timeframes (1M/W1 → D1 → H4), **Narrative (A→B)**, Narrative Algorithm
(4 питання), Trading Strategy Card (Inputs → Narrative → TDA → SL → Execution → Position-Adding).
Питання було пряме: **це механічна система чи треба думати самому?**

**Відповідь, підкріплена даними: система на ~80% ДИСКРЕЦІЙНА.** Її ядро — «Narrative»: трейдер сам
вирішує, куди ринок іде (A→B), де ліквідність, який сценарій старший. Це судження, а не правило —
**його не можна забектестити**, бо немає однозначного коду, який відтворює «куди я думаю піде ціна».

Щоб не відмахуватись словами, я **механізував САМУ СУТЬ** Narrative-розвороту: Premium/Discount через
dealing range (40 барів) + зняття ліквідності останнього свінга (sweep фрактала) + закриття назад
у зону (rejection) → вхід у бік протилежної зони, RR2, чесні витрати, IS/OOS.

**Результат — НЕГАТИВНЕ очікування майже скрізь:**

| ТФ | BTC | ETH | SOL |
|----|-----|-----|-----|
| D1 | −0.309R (WR 25%) | +0.166R (WR 41%) | −0.163R (WR 30%) |
| H4 | −0.045R (WR 35%) | −0.157R (WR 31%) | −0.090R (WR 33%) |

**IS/OOS на D1 (3 монети разом): IS −0.195R / OOS −0.129R.** Плюс лише в 1 клітинці з 6 (ETH D1) —
це шум, не едж. Для порівняння: FVG +0.28R, KC +0.28R — справжній едж виглядає інакше.

**Чому так:** це повністю узгоджується з усіма попередніми тестами реверсів у цьому проєкті —
CHoCH/BOS-розворот (§11) і sweep-реверс давали мінус. **Механічний розворот проти руху на крипті
не має статистичного еджу.** Едж AlexxxFX (якщо він у нього є) живе саме у дискреційному виборі
сетапів очима, який цей тест відтворити не може.

**Підсумок:**
- Як **алгоритм для бота/механічної торгівлі** — система AlexxxFX **не працює** (ядро не механізується,
  а його механічна сутність дає мінус). Торгувати її можна тільки **руками, дискреційно**.
- **Єдиний цінний механізований примітив з усієї SMC — це FVG**, і ми його вже витягли та
  провалідували (§27). Це і є чесний максимум, який можна взяти з цієї школи у формат правил.
- Рекомендація без змін: **KC (breakout) + FVG (retest)** як механічна основа; решту SMC — лише
  як ручний контекст, якщо тобі так зручніше дивитись ринок.
...
quant/
  ...
  23_reduce_dd.py 24_dd_reduction_plot.py  зменшення DD (фільтри+овердрайви) — §14
  25_realism_audit.py      аудит реалістичності (витрати/MTM/MC/крах)  — §15
  26_concentration.py 27_concentration_plot.py  розподіленість еджу    — §16
  28_regime_analysis.py 29_regime_plot.py  бичка/ведмежка/боковик       — §17
  30_flat_filter.py        тест флет-фільтрів (відхилено)               — §18
  31_monthly_consistency.py  місячна стабільність, MA, RR, к-сть монет  — §19
  ...
  41_smc_narrative_test.py  механізація суті Narrative AlexxxFX (мінус)  — §28
